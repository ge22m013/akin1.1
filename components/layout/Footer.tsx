...
const SocialIcon: React.FC<{ href: string, children: React.ReactNode, label: string }> = ({ href, children, label }) => (
  <a href={href} className="text-gray-400 hover:text-akin-cyan transition-colors" aria-label={label}>
    {children}
  </a>
);
      ...
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-8">
          <div className="space-y-4 text-center sm:text-left">
            <AkinLogo className="h-8 text-akin-text-dark mx-auto sm:mx-0" />
        ...
        <div className="mt-12 border-t border-gray-200 pt-8 flex flex-col-reverse sm:flex-row items-center justify-between gap-4">
            ...
            <nav className="flex flex-wrap justify-center gap-x-6 gap-y-2" aria-label="Footer">
                <Link to="/privacy-policy" className="text-sm hover:text-akin-cyan transition-colors">Privacy Policy</Link>
                <Link to="/careers" className="text-sm hover:text-akin-cyan transition-colors">Careers</Link>
                <Link to="/contact" className="text-sm hover:text-akin-cyan transition-colors">Contact</Link>
                <Link to="/sitemap" className="text-sm hover:text-akin-cyan transition-colors">Sitemap</Link>
            </nav>
        </div>
      </div>
    </footer>
