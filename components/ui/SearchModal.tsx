        ...
        <span>
            {parts.map((part, i) =>
                part.toLowerCase() === highlight.toLowerCase() ? (
                    <strong key={i} className="font-bold text-akin-cyan">{part}</strong>
                ) : (
                    part
                )
            )}
          ...
          >
            <div className="relative">
              ...
              <input
                ...
                className="w-full border-0 border-b border-gray-200 py-4 pl-11 pr-12 text-lg text-akin-text-dark placeholder:text-gray-400 focus:ring-0"
                        ...
                        <ul>
                            {filteredResults.map(item => (
                                <li key={item.path}>
                                    ...
                                    >
                                        <p className="text-sm text-akin-cyan">{highlightText(item.breadcrumb, query)}</p>
                                        <p className="font-semibold text-akin-text-dark">{highlightText(item.title, query)}</p>
                                    </Link>
                                </li>
                            ))}
                        </ul>
                    ) : (
                        <p className="p-8 text-center text-akin-text-light">No results found for "{query}"</p>
                    )}
                </div>
            )}
          </motion.div>
