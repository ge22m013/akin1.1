...
type Insight = Content & { type: string, topic: string, href?: string };
  ...
  return (
    ...
    >
      <Card className="flex flex-col w-full">
        ...
        {insight.href ? (
            <Link to={insight.href} className="mt-4 pt-4 border-t border-gray-200 font-semibold text-akin-cyan hover:text-akin-text-dark text-left w-full transition-colors">
                Read More &rarr;
            </Link>
        ) : (
            <button onClick={() => onInsightClick(insight)} className="mt-4 pt-4 border-t border-gray-200 font-semibold text-akin-cyan hover:text-akin-text-dark text-left w-full transition-colors">
                Read More &rarr;
            </button>
        )}
      </Card>
