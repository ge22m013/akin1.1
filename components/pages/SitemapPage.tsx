...
const velocityLinks = [
  ...
  { name: 'Collaborate & Evolve', href: '/velocity-approach/collaborate-evolve', isSub: true },
];
...
const SitemapSection: React.FC<{ title: string; links: { name: string; href: string; isSub?: boolean }[] }> = ({ title, links }) => (
  <div>
    <h3 className="text-base font-semibold text-akin-text-dark tracking-wider uppercase">{title}</h3>
    <ul className="mt-4 space-y-3">
      {links.map((item) => (
        <li key={item.name} className={item.isSub ? 'ml-4' : ''}>
          <Link to={item.href} className="text-base text-akin-text-light hover:text-akin-cyan transition-colors">{item.name}</Link>
        </li>
      ))}
    </ul>
  </div>
